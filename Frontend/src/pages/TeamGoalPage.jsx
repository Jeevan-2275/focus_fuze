...
const TeamGoalPage = () => {
  ...
  const initializeSocket = () => {
    ...
    socketRef.current.on("connect", () => console.log("Connected to Socket.IO", socketRef.current.id));
    ...
    socketRef.current.on("signal", (data) => {
      console.log("Received signal from", data.from, "to", socketRef.current.id);
      if (peers[data.from]) {
  ...
  const createPeer = (userId, callerId, recipientId, initiator, signal) => {
    console.log(`Creating peer for ${userId}, initiator: ${initiator}, caller: ${callerId}, recipient: ${recipientId}`);
    const peer = new SimplePeer({
      initiator: initiator || callerId === socketRef.current.id,
      trickle: false,
      stream: streamRef.current,
    });

    if (signal) {
      console.log("Signaling peer with initial signal");
      peer.signal(signal);
    }

    peer.on("signal", (signal) => {
      console.log(`Sending signal from ${callerId} to ${recipientId}`);
      socketRef.current.emit("signal", { to: recipientId, signal });
    });
          ...
          {view === "meeting" && selectedGoal && (
            <div className="team-goal-meeting-view">
              ...
              <div className="video-container">
                ...
                {Object.entries(peers).map(([peerUserId, { stream }]) => {
                  const participant = participants.find((p) => p.userId === peerUserId);
                  if (!participant || !stream) return null;
                  return (
                    <div key={`${peerUserId}-${Date.now()}`} className="video-wrapper">
                      <Typography variant="body2" style={{ textAlign: "center" }}>
